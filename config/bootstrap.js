...
module.exports.bootstrap = async function() {
  ...
  var canal1 = await Room.create({name: 'CANAL CHIDO'}).fetch()
  await Room.create({name: 'CANAL FEO'})
  await Room.create({name: 'CANAL COOL'})

  await Message.create({text:'ALV PERRISMO DESGRACIACOD', owner: canal1.id})
  await Message.create({text:'ALV PERRISMO DESGRACIACOD', owner: canal1.id})

  // await Room.addToCollection(rory.id, 'message').members([raquel.id, rachael.id, mike.id]);
